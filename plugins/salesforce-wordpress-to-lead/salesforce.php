...
function salesforce_cc_user( $post, $options, $form_id = 1 ){
	...
	$message = 'Thank you for contacting Phase Genomics! We will be in touch shortly. For your reference, here is the information which was sent to us:'."\r\n\r\n";
	...
	foreach($post as $name => $value){
		...
		if( !empty( $name ) && !empty( $value ) && !empty( $label ) ){
			if( $name == 'description') {
				$message .= stripslashes($label).':'."\r\n".salesforce_maybe_implode(',', $value)."\r\n";
			}else{
...
function salesforce_cc_admin( $post, $options, $form_id = 1, $subject = '', $append = '' ){
	...
	//From Shawn: commenting out obvious Salesforce branding in subject
	//if( !$subject )
	...
	//From Shawn: make the email set up for replying to the person who submitted it
	$entered_first_name = '';
	$entered_last_name = '';
	$entered_email = '';
	foreach($post as $name=>$value){
		if( !empty($value) && $name == 'first_name' )
			$entered_first_name = salesforce_maybe_implode( ';', $value );
		if( !empty($value) && $name == 'last_name' )
			$entered_last_name = salesforce_maybe_implode( ';', $value );
		if( !empty($value) && $name == 'email' )
			$entered_email = salesforce_maybe_implode( ';', $value );
	}

	$from_name = $entered_first_name.' '.$entered_last_name;
	if( empty($entered_from_name) && empty($entered_last_name) ) 
		$from_name = salesforce_get_option( 'emailfromname', $form_id, $options );
	if( !$from_name )
		$from_name = get_bloginfo('name');

	$from_email = $entered_email;
	...
	foreach($post as $name=>$value){
		...
		if( !empty($value) && ! empty( $label ) ){
			if( $label != '' && $name == 'subject' )
				$subject .= ': '. salesforce_maybe_implode( ';', $value );
			if( $label != '' && $name == 'description' )
				$message .= salesforce_maybe_implode( ';', $value ) . "\r\n";
		}
	}

	//From Shawn: commenting out obvious Salesforce branding
	//if ( $post['lead_source'] ) {
	...
	//cc the user on the admin email if they asked for a copy
	if (isset( $_POST['w2lcc'] ) && $_POST['w2lcc'] == 1){
